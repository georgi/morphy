 ...
 *
 * Imports ChessModule + AnalysisModule to inject ChessService, AnalysisService,
 * and CoachService. The tools operate on the open game the client sends by value
 * with each message (no shared game store, no library). The {@link AGENT_HARNESS}
 * provider selects the Pi or Claude backend from the environment at boot.
 */
@Module({
