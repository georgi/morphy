...
export interface EngineLine { pv: string[]; scoreCp: number | null; mate: number | null; rank: number; }
export interface EngineEval { fen: string; bestMove: string | null; lines: EngineLine[]; depth: number; }

export interface Move { ply: number; moveNumber: number; color: Color; san: string; uci: string; fenBefore: string; fenAfter: string; }
export interface MoveEval { ply: number; san: string; scoreCpBefore: number | null; scoreCpAfter: number | null; cpLoss: number; classification: MoveClassification; bestMove: string | null; bestLine: string[]; }
export interface GameHeaders { event?: string; white?: string; black?: string; result?: string; date?: string; eco?: string; opening?: string; [k: string]: string | undefined; }
export interface Game { id: string; headers: GameHeaders; startFen: string; moves: Move[]; analysis?: MoveEval[]; }
...
export interface ImportGameRequest { pgn?: string; fen?: string; }
export interface AnalyzePositionRequest { fen: string; depth?: number; multipv?: number; }
export interface AnalyzeGameRequest { gameId: string; depth?: number; }
export interface AgentMessageRequest { text: string; gameId?: string; ply?: number; }
...
export type AgentEvent =
  ...
  | { type: 'coach_question'; gameId: string; ply: number; fen: string; sideToMove: Color; index: number; total: number }
  | { type: 'coach_reveal'; ply: number; bestMove: string | null; bestLine: string[]; playedSan: string; userSan?: string; verdict: 'correct' | 'close' | 'off' | 'revealed'; evalText: string }
  | { type: 'done' }
  | { type: 'error'; message: string };

export const CLASSIFY_THRESHOLDS = { inaccuracy: 50, mistake: 100, blunder: 300 } as const;
